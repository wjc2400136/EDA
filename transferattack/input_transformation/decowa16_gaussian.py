...
class DeCowABM(MIFGSM):
    ...
    def elastic_warp(self, x, use_center_grid=False):
        """
        对图像进行弹性形变（使用零填充+高斯噪声扩展技术,填充量基于控制点的最大偏移值）
        ...
        # 【修改】使用零填充扩展图像边界
        padded_img = F.pad(x, (pad_w, pad_w, pad_h, pad_h), mode='constant', value=0)
        new_h, new_w = h + 2 * pad_h, w + 2 * pad_w

        # 【修改】在填充区域添加高斯噪声
        noise = torch.randn_like(padded_img) * 0.05  # 噪声标准差设为0.1
        # 创建一个mask，标记填充区域（非原始图像区域）
        ...
        mask[:, :, :, -pad_w:] = 1  # 右
        # 将噪声加到填充区域（零像素区域）
        padded_img = padded_img + noise * mask
