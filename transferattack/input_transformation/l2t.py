...
def _dct_matrices(n, dtype, device):
    key = (n, dtype, device)
    if key not in _DCT_MATRIX_CACHE:
        sample_index = torch.arange(n, dtype=dtype, device=device)
        frequency_index = torch.arange(n, dtype=dtype, device=device).unsqueeze(1)
        dct_matrix = 2.0 * torch.cos(
            math.pi * frequency_index * (2.0 * sample_index + 1.0) / (2.0 * n)
        )
        inverse_dct_matrix = torch.linalg.inv(dct_matrix)
        _DCT_MATRIX_CACHE[key] = (dct_matrix, inverse_dct_matrix)
...
class ssm():
    def __init__(self, rho=0.5, num_spectrum=10):
        ...
        self.dct_backend = os.environ.get("L2T_DCT_BACKEND", "fft").strip().lower()

    def dct_matmul(self, x):
        n = x.shape[-1]
        dct_matrix, _ = _dct_matrices(n, x.dtype, x.device)
        return torch.matmul(x, dct_matrix.t())

    def idct_matmul(self, x):
        n = x.shape[-1]
        _, inverse_dct_matrix = _dct_matrices(n, x.dtype, x.device)
        return torch.matmul(x, inverse_dct_matrix.t())
    ...
    def dct_2d(self, x, norm=None):
        ...
        """
        dct_1d = self.dct_matmul if self.dct_backend == "matmul" else self.dct
        X1 = dct_1d(x) if self.dct_backend == "matmul" else dct_1d(x, norm=norm)
        X2_input = X1.transpose(-1, -2)
        X2 = dct_1d(X2_input) if self.dct_backend == "matmul" else dct_1d(X2_input, norm=norm)
    ...
    def idct_2d(self, X, norm=None):
        ...
        Return:
            the DCT-II of the signal over the last 2 dimensions
        """
        idct_1d = self.idct_matmul if self.dct_backend == "matmul" else self.idct
        x1 = idct_1d(X) if self.dct_backend == "matmul" else idct_1d(X, norm=norm)
        x2_input = x1.transpose(-1, -2)
        x2 = idct_1d(x2_input) if self.dct_backend == "matmul" else idct_1d(x2_input, norm=norm)
        return x2.transpose(-1, -2)
...
class L2T(Attack):
    ...
    Official arguments:
        epsilon=16/255, alpha=epsilon/epoch=1.6/255, epoch=10, decay=1, num_scale=2
    ...
    """
    def __init__(self, model_name, epsilon=16/255, alpha=1.6/255, epoch=10, decay=1., num_scale=2,
                 targeted=False, random_start=False, norm='linfty', loss='crossentropy', device=None, attack='L2T',seed=42, **kwargs):
            ...
            for e in range(self.epoch):
                ...
                for i in range(self.num_scale):
                    ...
                    cudnn_context = (
                        ...
                        if torch.cuda.is_available() and _env_flag("L2T_DISABLE_CUDNN", False)
