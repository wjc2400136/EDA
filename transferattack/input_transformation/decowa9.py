...
class DeCowABM(MIFGSM):
    ...
    Arguments:
        ...
        noise_type (str): 噪声类型，'gaussian'或'uniform'
        noise_values (list): 噪声值列表
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 use_dual_grid=True, noise_type='gaussian', noise_values=[(16 / 255)/6], **kwargs):
        ...
        self.noise_values = noise_values if noise_values is not None else []
        ...
        for epoch_idx in range(self.epoch):
            ...
            for warp_idx in range(self.num_warping):
                ...
                if self.noise_type == 'gaussian':
                    # 高斯噪声：从噪声值列表中随机选择一个标准差
                    if self.noise_values:
                        chosen_std = random.choice(self.noise_values)
                        noise = torch.randn_like(mixed_img) * chosen_std
                        mixed_img_with_noise = mixed_img + noise
                        mixed_img = torch.clamp(mixed_img_with_noise, 0.0, 1.0)
                elif self.noise_type == 'uniform':
                    # 均匀噪声：从噪声值列表中随机选择一个范围
                    if self.noise_values:
                        chosen_range = random.choice(self.noise_values)
                        noise = torch.rand_like(mixed_img) * (2 * chosen_range) - chosen_range
                        mixed_img_with_noise = mixed_img + noise
                        mixed_img = torch.clamp(mixed_img_with_noise, 0.0, 1.0)
