...
class DeCowABM(MIFGSM):
    ...
    Arguments:
        ...
        use_transform (bool): whether to use random transforms (default: True)
        use_padding (bool): whether to use padding (default: True)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 use_dual_grid=True, exclude_transform=None, use_transform=True, use_padding=True, **kwargs):
        ...
        self.use_transform = use_transform  # 新增参数
        self.use_padding = use_padding  # 新增参数
    ...
    def elastic_warp(self, x, use_center_grid=False):
        ...
        if self.use_padding:
            ...
            # 不进行填充，直接在原始图像上应用TPS变换
            # 创建针对原始图像的TPS变换器
            tps_original = TPS(size=(h, w), device=self.device)

            # 应用TPS变换，使用原始控制点坐标
            warped_grid = tps_original(X[None, ...], Y[None, ...])
            warped_grid = warped_grid.repeat(x.shape[0], 1, 1, 1)
            warped_grid = warped_grid.view(n, h, w, 2)

            # 在原始图像上应用形变
            warped_img = torch.grid_sampler_2d(x, warped_grid, 0, 0, False)
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            for warp_idx in range(self.num_warping):
                ...
                # 【核心修改】根据use_transform参数决定是否应用变换
