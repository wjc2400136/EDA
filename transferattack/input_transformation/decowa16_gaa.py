...
class DeCowABM(MIFGSM):
    ...
    融合GAA (Gradient Aggregation Attack) 方法
    Arguments:
        ...
        use_gaa (bool): whether to use GAA gradient calculation (default: True)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 # GAA相关参数
                 rho=1.6 / 255, lambda_param=0.2, xi=0.1, N=20, use_gaa=True, **kwargs):
    ...
    def calculate_gradient(self, x, label, delta):
        """
    ...
    def get_aggregated_gradient(self, data, delta, label, use_center_grid=False, **kwargs):
        """
        Calculate the aggregated gradient (GAA方法)
        """
        ...
        for i in range(self.N):
            ...
            x_prime = transform(x_prime)

            # 应用弹性形变
            x_prime = self.elastic_warp(x_prime, current_use_center_grid)

            # Calculate gradient g' = ∇_x L(x', y_t; θ)
            g_prime = self.calculate_gradient(x_prime, label, delta)
            ...
            # 应用颜色变换
            transform = random.choice(all_transforms)
            x_hat = transform(x_hat)

            # 应用弹性形变
            x_hat = self.elastic_warp(x_hat, current_use_center_grid)

            # Calculate gradient g_hat = ∇_x L(x_hat, y_t; θ)
            g_hat = self.calculate_gradient(x_hat, label, delta)
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            if self.use_gaa:
                # 使用GAA方法计算聚合梯度
                g_bar = self.get_aggregated_gradient(data, delta, label, use_center_grid)
