...
def apply_spatial_transform(x: torch.Tensor, r: float, apply_prob: float = 1.0):
    """
    随机应用两种空间变换模式之一：
    - s < 1: 缩小+填充（SID风格），缩小后的图像在画布上随机位置放置
    - s > 1: 放大+裁剪（I-C风格），裁剪位置随机
    :param x: 输入图像 (B, C, H, W)
    :param r: 缩放范围参数，s从[1-r/2, 1+r/2]均匀采样
    :param apply_prob: 应用变换的概率
    :return: 变换后的图像
    """
    if apply_prob < torch.rand(1, device=x.device).item():
        return x

    B, C, H, W = x.shape

    # 【修复】使用torch.rand替代.uniform_，避免参数解析问题
    s_min = 1 - r / 2
    s_max = 1 + r / 2
    s = (s_max - s_min) * torch.rand(1, device=x.device).item() + s_min

    if s < 1:  # 缩小+填充模式（SID风格）
        # 缩小图像
        ...
        # 随机选择放置位置（确保随机性）
        pad_top = torch.randint(0, max_top + 1, (1,), device=x.device).item()
        pad_left = torch.randint(0, max_left + 1, (1,), device=x.device).item()
        pad_bottom = max_top - pad_top
        pad_right = max_left - pad_left

        # 填充到原始尺寸，保持随机位置
        transformed = F.pad(rescaled,
                            [pad_left, pad_right, pad_top, pad_bottom],
                            mode='constant', value=0)

    elif s > 1:  # 放大+裁剪模式（I-C风格）- 裁剪位置随机化
        # 放大图像
        new_h, new_w = int(H * s), int(W * s)
        rescaled = F.interpolate(x, size=[new_h, new_w], mode='bilinear', align_corners=False)
        ...
        # 随机选择裁剪位置（不再是固定中心）
        top = torch.randint(0, max_top + 1, (1,), device=x.device).item()
        left = torch.randint(0, max_left + 1, (1,), device=x.device).item()

        # 创建显式可微采样网格（核心修改）
        # 计算裁剪区域在归一化坐标系中的边界
        x_start = 2.0 * left / (new_w - 1) - 1.0
        x_end = 2.0 * (left + W - 1) / (new_w - 1) - 1.0
        y_start = 2.0 * top / (new_h - 1) - 1.0
        y_end = 2.0 * (top + H - 1) / (new_h - 1) - 1.0

        # 生成均匀采样网格
        grid_x = torch.linspace(x_start, x_end, W, device=x.device)
        grid_y = torch.linspace(y_start, y_end, H, device=x.device)
        grid_y, grid_x = torch.meshgrid(grid_y, grid_x, indexing='ij')
        grid = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(0).repeat(B, 1, 1, 1)

        # 使用grid_sample实现可微的随机裁剪
...
class DeCowABM(MIFGSM):
    ...
    Arguments:
        ...
        r (float): spatial transform range parameter (default: 0.1)
        spatial_transform_prob (float): probability of applying spatial transform (default: 1.0)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 block_shuffle_high=0.9, num_block=2,
                 r=0.25, spatial_transform_prob=1.0, **kwargs):  # 【新增】空间变换参数
        ...
        # 【新增】空间变换参数
        self.r = r  # 缩放范围参数
        self.spatial_transform_prob = spatial_transform_prob  # 应用空间变换的概率
    ...
    def elastic_warp(self, x, use_center_grid=False):
        ...
        # 根据控制点的最大偏移值计算填充量，此时要求图像的宽与高相等
        max_offset_norm = 0.5 * self.noise_scale

        # 将 float 结果显式转换为 Tensor
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            for warp_idx in range(self.num_warping):
                ...
                # 【新增】应用随机空间变换（缩小+填充 或 放大+裁剪）
                final_img = apply_spatial_transform(shuffled_img, self.r, self.spatial_transform_prob)

                # 使用最终变换后的图像计算梯度
                logits = self.get_logits(final_img)
