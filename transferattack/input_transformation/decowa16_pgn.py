...
class DeCowABM(MIFGSM):
    ...
    融合PGN (Penalizing Gradient Norm) 方法
    Arguments:
        ...
        use_pgn (bool): whether to use PGN gradient calculation (default: True)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 # PGN相关参数
                 beta=3.0, gamma=0.5, num_neighbor=20, use_pgn=True, **kwargs):
    ...
    def get_averaged_gradient(self, data, delta, label, use_center_grid=False, **kwargs):
        """
        Calculate the averaged updated gradient (PGN方法)
        """
        averaged_gradient = 0
        for _ in range(self.num_neighbor):
            ...
            x_near = transform(x_near)

            # 应用弹性形变
            x_near = self.elastic_warp(x_near, current_use_center_grid)
            ...
            # 计算x_near的梯度，保留计算图
            g_1 = torch.autograd.grad(loss, delta, retain_graph=True, create_graph=False)[0]
            ...
            # 应用颜色变换
            transform = random.choice(all_transforms)
            x_next = transform(x_next)

            # 应用弹性形变
            x_next = self.elastic_warp(x_next, current_use_center_grid)
            ...
            # 计算x_next的梯度
            g_2 = torch.autograd.grad(loss, delta, retain_graph=False, create_graph=False)[0]
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            if self.use_pgn:
                # 使用PGN方法计算平均梯度
                averaged_gradient = self.get_averaged_gradient(data, delta, label, use_center_grid)
