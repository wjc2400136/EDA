...
class DeCowABM(MIFGSM):
    ...
    融合VMI-FGSM (Variance Tuning MI-FGSM) 方法
    Arguments:
        ...
        use_vmifgsm (bool): whether to use VMI-FGSM gradient calculation (default: True)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 # VMI-FGSM相关参数
                 beta=1.5, num_neighbor=20, use_vmifgsm=True, **kwargs):
    ...
    def get_variance(self, data, delta, label, cur_grad, momentum, use_center_grid=False, **kwargs):
        """
        Calculate the gradient variance (VMI-FGSM方法)
        """
        grad = 0
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            if self.use_vmifgsm:
                ...
                # 计算方差
                variance = self.get_variance(data, delta, label, grad, momentum, use_center_grid)
