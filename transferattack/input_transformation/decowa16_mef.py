...
class DeCowABM(MIFGSM):
    ...
    融合MEF (Maximin Expected Flatness)方法提升对抗样本的迁移性
    ...
    Arguments:
        ...
        use_mef (bool): whether to use MEF method (default: True)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 # MEF相关参数
                 num_neighbor=10, gamma=2., kesai=0.15, inner_decay=0.9, use_mef=True, **kwargs):
    ...
    def get_points_gradient(self, data, delta, label, use_center_grid=False, transform=None):
        """
        ...
        for i in range(self.num_neighbor):
            ...
            # 应用弹性形变
            warped_img = self.elastic_warp(x_min, use_center_grid)
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            if self.use_mef:
                ...
                gradient = self.get_points_gradient(data, sample_delta, label, use_center_grid, transform)

                # Update gradient for previous gradient inversion approximation
                grad_pgia = ((gradient / torch.mean(torch.abs(gradient), (2, 3, 4),
                                                    keepdim=True)).detach() - self.inner_decay * grad_pgia)
