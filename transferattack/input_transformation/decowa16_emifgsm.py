...
class DeCowABM(MIFGSM):
    ...
    融合EMI-FGSM (Enhanced Momentum Iterative FGSM) 方法
    Arguments:
        ...
        use_emifgsm (bool): whether to use EMI-FGSM gradient calculation (default: True)
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1.,
                 ...
                 # EMI-FGSM相关参数
                 num_sample=11, radius=7, sample_method='linear', use_emifgsm=True, **kwargs):
    ...
    def transform(self, x, grad, use_center_grid=False, **kwargs):
        """
        Admix the input for EMI-FGSM Attack
        """
        ...
        for factor in factors:
            # 随机选择控制点布局（仅在启用双网格时）
            current_use_center_grid = use_center_grid
            if self.use_dual_grid and self.center_grid.numel() > 0:
                current_use_center_grid = random.random() < 0.5

            # 创建采样点
            sample = x + factor * self.alpha * grad
            ...
            # 应用弹性形变
            sample = self.elastic_warp(sample, current_use_center_grid)
            transformed_samples.append(sample)

        return torch.concat(transformed_samples)

    def get_loss(self, logits, label):
        """
        Calculate the loss
        """
        return -self.loss(logits, label.repeat(self.num_sample)) if self.targeted else self.loss(logits, label.repeat(
            self.num_sample))
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            ...
            # 根据use_emifgsm参数选择梯度计算方式
            if self.use_emifgsm:
                # 使用EMI-FGSM方法计算梯度

                # 获取增强的采样点
                transformed_samples = self.transform(data + delta, grad=bar_grad, use_center_grid=use_center_grid)

                # 计算输出
                logits = self.get_logits(transformed_samples)
