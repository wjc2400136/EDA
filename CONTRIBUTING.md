# Contributing

[English](CONTRIBUTING.md) | [简体中文](CONTRIBUTING.zh-CN.md)

Contributions that improve reproducibility, portability, documentation, or
error reporting are welcome.

Before opening an issue, include the operating system, Python/PyTorch/CUDA/timm
versions, GPU model, command, dataset schema, and complete traceback. Do not
upload dataset images, third-party checkpoints, API credentials, or provider
responses containing private information.

For pull requests:

1. Keep changes scoped to the reported problem.
2. Preserve the paper's default attack and averaging protocol.
3. Add or update documentation when changing a CLI argument.
4. Run Python syntax checks and the relevant small-scale test.
5. Do not replace upstream license or attribution notices.

Security-sensitive reports, especially accidental credential exposure, should
be sent privately to the repository maintainers rather than posted publicly.
